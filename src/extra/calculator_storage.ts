...
export function caluclate_raid(
  ...
  switch (raid_config) {
    case RAIDEnums.R0:
      return parseFloat(
        storage_netto(storage_amount, storage_size).toFixed(decimals)
      );
    case RAIDEnums.R1:
      if (storage_amount >= 2)
        return parseFloat(
          (storage_netto(storage_amount, storage_size) / 2).toFixed(decimals)
        );
      break;
    case RAIDEnums.R5:
      if (storage_amount >= 3)
        return parseFloat(
          (
            storage_netto(storage_amount, storage_size) *
            (1 - 1 / storage_amount)
          ).toFixed(decimals)
        );
      break;
